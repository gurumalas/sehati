...
if (isset($_GET['act']) && isset($_GET['ref'])) {
    ...
    if ($act == "clear") {
        ...
        $cart = unserialize(serialize($_SESSION['cart']));
        $nik = $_SESSION['nik'];
        $ttotal += $cart[$i]->harga * $cart[$i]->quantity;
        $kd_transaksi = autonumber("invoice", "kd_transaksi", "5", "T");
        $totalsemua = $_POST['upah'];
        $sqltransaksi = mysqli_query($koneksi, "INSERT INTO transaksi(nik,kd_transaksi,tglt,ongkir,total,totalsemua)
VALUES ('$nik','$kd_transaksi','$tgl','$ttotal','$subtotal','$totalsemua')") or die(mysqli_error($koneksi));
        $status = 0;
        $max=count($cart);
        for($i=0;$i<$max;$i++){
            $ttotal += $cart[$i]->harga * $cart[$i]->quantity;
                        ...
                        <form method="post">
                            <tr>
                                    ...
                                    <select name="onkir" id="onkir" onchange="changeValue(this.value)">
                                        ...
                                        while ($row = mysqli_fetch_array($result)) {
                                            ...
                                            echo '<option value="' . $row['id_onkir'] . '">' . $row['kec'] . '-' . $row['ongkir'] . '</option>
                                                    ';
                                            $jsArray .= "dataongkir['" . $row['id_onkir'] . "'] = {upah:'" . addslashes($hasill). "',ttotal:'".addslashes($row['ongkir'])."'};\n";
                                ...
                                <td colspan="" style="text-align:right; font-weight:bold">
                                    <button id="saveimg" class="button btn-primary" src="images/save.png" name="update" alt="Upadate QTY ">Update QTY</button>
                                    <button id="saveimg" class="button btn-primary" src="images/save.png" value="Refresh Halaman" onclick="window.location.href=window.location.href;">Refresh</button>
                                </td>
