...
class Cart
{
     ...
     *
     * @param string $kd_produk
     * @param array  $attributes
     *
     * @return bool
     */
    public function isItemExists($kd_produk, $attributes = [])
    {
        $attributes = (is_array($attributes)) ? array_filter($attributes) : [$attributes];

        if (isset($this->items[$kd_produk])) {
            $hash = md5(json_encode($attributes));
            foreach ($this->items[$kd_produk] as $item) {
                if ($item['hash'] == $hash) {
                    return true;
                }
            }
     ...
     * @return bool
     */
    public function add($kd_produk, $quantity = 1, $attributes = [])
    {
        ...
        if (isset($this->items[$kd_produk])) {
            foreach ($this->items[$kd_produk] as $index => $item) {
                if ($item['hash'] == $hash) {
                    $this->items[$kd_produk][$index]['quantity'] += $quantity;
                    $this->items[$kd_produk][$index]['quantity'] = ($this->itemMaxQuantity < $this->items[$kd_produk][$index]['quantity'] && $this->itemMaxQuantity != 0) ? $this->itemMaxQuantity : $this->items[$kd_produk][$index]['quantity'];
     ...
     * @return bool
     */
    public function update($kd_produk, $quantity = 1, $attributes = [])
    {
        $quantity = (preg_match('/^\d+$/', $quantity)) ? $quantity : 1;

        if ($quantity == 0) {
            $this->remove($kd_produk, $attributes);
     ...
     * @return bool
     */
    public function remove($kd_produk, $attributes = [])
    {
        if (!isset($this->items[$kd_produk])) {
            return false;
        }
        ...
        foreach ($this->items[$kd_produk] as $index => $item) {
            if ($item['hash'] == $hash) {
                unset($this->items[$kd_produk][$index]);
